...
public class HttpSocketConnection {
    ...
    //
    public HttpSocketConnection(String url) throws MalformedURLException {
        this(new URL(url));
    }
    ...
    public void connect() throws IOException {
        ...
        //
        if (!mProperties.containsKey("accept")) {
            ArrayList<String> al = new ArrayList<>();
            al.add("*/*");
            mProperties.put("accept", al);
        }
        //
        ...
        if (this.usingProxy()) {
            ...
            if (usingProxyAuth()) {
                ...
                sbRequest.append("proxy-authorization")
                        .append(": ")
                        .append(proxyAuthEncode)
                        .append(NEWLINE_STR);
            }
        ...
        socket = sslSocket;
        //{METHOD} {FILEPATH} HTTP/1.0\r\n
        String filePath = url.getFile();
        filePath = filePath.equals("") ? "/" : filePath;
        sbRequest.append(getRequestMethod())
                .append(" ")
                .append(filePath)
                .append(" HTTP/1.1")
                .append(NEWLINE_STR);
        //Host: {HOST}\r\n
        sbRequest.append("host: ")
                .append(host)
                .append(NEWLINE_STR);
        //{KEY}: {VALUE}\r\n
        mProperties.entrySet().forEach(mProperty -> {
            sbRequest.append(mProperty.getKey())
                    .append(": ")
                    .append(String.join("; ", mProperty.getValue()))
                    .append(NEWLINE_STR);
        });
        //Connection: close\r\n
        sbRequest.append("connection: close")
    ...
    public InputStream getInputStream() throws IOException {
        ...
        while ((strLine = br.readLine()) != null) {
            if (intLine == 0) {
                ...
                String[] pairHeaders = strLine.split(":");
                addHeaderField(pairHeaders[0].trim(), pairHeaders[1].trim());
                intLine++;
            } else {
                strResponseBody.append(strLine).append("\n");
            }
        }
        responseBody = strResponseBody.toString().replaceAll("\\n$", "");
